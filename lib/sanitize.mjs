...
export function sanitize(html) {
  ...
  // 3) forms + form-plugin wrappers (display-only mirror)
  $('form, [class*="gform"], [id*="gform"], [class*="forminator"], [id*="forminator"]').remove()
  ...
  $('head').append(
    '<style id="bcc-overrides">' +
      '.wp-grid-builder:not(.wpgb-template),.wpgb-grid,.wpgb-facet,.wpgb-content{opacity:1 !important;}' +
    '</style>'
  )
