...
// Cloudflare "Email Address Obfuscation" replaces emails with a placeholder + a
// decoder script. We strip scripts, so decode the data-cfemail hex ourselves
// (first byte is an XOR key for the rest). This is a display-only staff mirror, so
// showing real addresses is exactly what's wanted.
...
function decodeCloudflareEmails($) {
  // Elements Cloudflare tags with the encoded address.
  $('.__cf_email__, [data-cfemail]').each((_, el) => {
    const $el = $(el)
    const email = cfDecode($el.attr('data-cfemail'))
    if (!email) return
    if ($el.is('a')) {
      $el.attr('href', 'mailto:' + email).removeClass('__cf_email__').removeAttr('data-cfemail').text(email)
    } else {
      $el.replaceWith(email)
    }
  })
  // Anchors whose href is the Cloudflare email-protection link (#<hex>).
  $('a[href*="/cdn-cgi/l/email-protection"]').each((_, el) => {
    const $el = $(el)
    const m = ($el.attr('href') || '').match(/#([0-9a-fA-F]+)/)
    if (!m) return
    const email = cfDecode(m[1])
    if (!email) return
    $el.attr('href', 'mailto:' + email)
    if (($el.text() || '').includes('[email')) $el.text(email)
  })
}
...
export function sanitize(html) {
  ...
  // 0) reveal Cloudflare-obfuscated email addresses (decoder script is stripped)
