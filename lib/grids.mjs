...
export function injectMarquee(html) {
  ...
  $('[data-ticker="1"]').each((_, el) => {
    const track = $(el).find('.oxy-dynamic-list').first()
    if (!track.length) return
    $(el).addClass('bcc-marquee')
    track.addClass('bcc-marquee-track')
    changed = true
  })
  if (!changed) return html
  $('head').append(`<style id="bcc-marquee-css">${MARQUEE_CSS}</style>`)
  $('body').append(`<script id="bcc-marquee-js">${MARQUEE_JS}</script>`)
  return $.html()
}

const MARQUEE_CSS = `
.bcc-marquee{overflow:hidden!important;width:100%}
.bcc-marquee .bcc-marquee-track{display:flex!important;flex-wrap:nowrap!important;width:max-content!important;align-items:center;will-change:transform;animation:bcc-marquee-scroll 45s linear infinite}
.bcc-marquee .bcc-marquee-track:hover{animation-play-state:paused}
.bcc-marquee .bcc-marquee-track>*{flex:0 0 auto!important;white-space:nowrap;padding:0 40px}
@keyframes bcc-marquee-scroll{from{transform:translateX(0)}to{transform:translateX(-50%)}}
`

const MARQUEE_JS = `(function(){
  document.querySelectorAll('.bcc-marquee .bcc-marquee-track').forEach(function(t){
    var items=Array.prototype.slice.call(t.children);
    if(!items.length) return;
    // Duplicate the item set once so the -50% loop is seamless.
    items.forEach(function(c){ t.appendChild(c.cloneNode(true)); });
  });
})();`
