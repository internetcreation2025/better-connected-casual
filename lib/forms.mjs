...
// Forms approved to be live on the mirror. Start with the myzone POC; expand once
// the pipeline is validated, then add the upload/signature/captcha forms later.
export const ENABLED_FORMS = new Set([3884])
...
export async function injectForms(html) {
  if (!html || html.indexOf('forminator-module-') === -1) return html
  const $ = cheerio.load(html)
  const shells = $('[id^="forminator-module-"]')
  if (!shells.length) return html

  let injected = false
  for (const el of shells.toArray()) {
