...
export const ENABLED_FORMS = new Set([
  ...
  37066, // campaign-feedback (single-file upload — works)
  3888, // portal-feedback (multiple-file upload — attachment pending)
  37894, // quarterly-nomination (multiple-file upload — attachment pending)
  37896, // special-recognition (multiple-file upload — attachment pending)
  36355, // fslt-newsletter (multiple-file upload — attachment pending)
...
function renderField(f) {
  ...
  switch (f.type) {
    ...
    case 'hidden':
      ...
      return ''
    case 'upload': {
      // Multiple-file fields need Forminator's async pre-upload flow (not yet wired),
      // so omit them rather than silently drop files. Single-file uploads work.
